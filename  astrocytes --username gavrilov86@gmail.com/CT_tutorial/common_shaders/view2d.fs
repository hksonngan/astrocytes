...
coord*=size;
	...
	h0/=size;
	h1/=size;
	float tex00 = texture2D(tex, h0).w;
	float tex10 = texture2D(tex, vec2(h1.x, h0.y)).w;
	float tex01 = texture2D(tex, vec2(h0.x, h1.y)).w;
	float tex11 = texture2D(tex, h1).w;
...
void main()
{
	float val = params.x>0.5 ? interpolate_bicubic(txt, text_coord.xy) : texture2D(txt, text_coord.xy).w;
	vec4 color = params.y>0.5 ? GetLevelColor(val).xyzw : vec4((val - min_level)/(max_level-min_level));
	color.y=0.0;
	color.z=0.0;
	//vec3 color =vec3((val - min_level)/(max_level-min_level));
